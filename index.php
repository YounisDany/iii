<?php
ob_start();
$token = "6662998188:AAENgmFaiICz6JjkmUeIOQP9ocpXQCu7oZQ";
define("API_KEY",$token);
echo file_get_contents("https://api.telegram.org/bot" . API_KEY . "/setwebhook?url=" . $_SERVER['SERVER_NAME'] . "" . $_SERVER['SCRIPT_NAME']);
function bot($method,$datas=[]){
$rdwan = http_build_query($datas);
$url = "https://api.telegram.org/bot".API_KEY."/".$method."?$rdwan";
$rdwan = file_get_contents($url);
return json_decode($rdwan);
}
...
if($update->callback_query ){
  if ($timer["acount"][$from_id] < time()) {
    if($update->callback_query->message->chat->id != $sudo and $update->callback_query->message->chat->id != 5975553574) {
$data = $update->callback_query->data;
$chat_id = $update->callback_query->message->chat->id;
$title = $update->callback_query->message->chat->title;
$message_id = $update->callback_query->message->message_id;
...
$adm = [ 
  'inline_keyboard'=>[
    ...
    [['text'=>"تعيين اقل عدد لتحويل النقاط",'callback_data'=>"sAKTHAR" ]],
    [['text'=>"اضافه او خصم نقاط",'callback_data'=>"coins" ], ['text'=>"تصفير نقاط شخص",'callback_data'=>"msfrn" ]], 
...
$admnb = [ 
  ...
  'inline_keyboard'=>[
    ...
    [['text'=>'تعين كليشه شراء النقاط' ,'callback_data'=>"setbuy"]],
  ...
  if($data == "settext"){
    if($chat_id == $sudo or $chat_id == 5975553574 ) {
      if(true){
        bot('EditMessageText',[
          'chat_id'=>$chat_id,
          'message_id'=>$message_id,
...
'chat_id'=>$chat_id, 
'text'=>"
*شروط استخدام البوت والخصوصية*

*قمنا بإضافة بعض القيود في البوت لضمان تجربة أفضل..*

`1- يمنع استخدام اكواد الهدايا في اكثر من حساب وتحويلها الى حسابك الاساسي`
`2- يمنع رشق قناة معينة أكثر من مرة خلال وقت قصير مالم يتم اكمال الطلب الاول لنفس القناه`
`3- يمنع رشق القنوات والبوتات الاباحية`
`4- يمنع استخدام البوت من أكثر من حساب`
`5- يمنع استخدام الثغرات (إن وجدت)`
`6- النصب والاحتيال في تحويل النقاط لمشترك آخر `

*في حال مخالفة الشروط او اساءة استخدام البوت يحق لنا :*

`    • تقييد حسابك دون إبلاغك`
`    • حذف عمليات الرشق التي قمت بها سابقا (مغادرة تلقائية للحسابات من القنوات والمجموعات التي قمت برشقها) `
",
'parse_mode'=>"markdown",
'reply_markup'=>json_encode([
     'inline_keyboard'=>[
...
SETJSON($rshq);
  ...
  $chabot = $rshq['cha']; if ($chabot == null){$chabot = "ckn1l";}
...
نقاطك في الموقع : *$flos$*
العمله : *$treqa*
اقل عدد لتحويل النقاط: *$AKTHAR*
",
'parse_mode'=>"markdown",
...
1 ⌯ دعوه الاصدقاء ‍🙋
2 ⌯ عجله الحظ 🎡
3 ⌯ شراء نقاط من المطور 📱
≪⌁⌁⌁⌁⌁ طريقه الرشق ⌁⌁⌁⌁≫
1 ⌯ اضغط على زر الخدمات
2 ⌯ اختر التطبيق الذي تريد الرشق له
3 ⌯ اختر الخدمة الذي تريدها
4 ⌯ قم بإختيار العدد 
5 ⌯ قم بإرسال الرابط المطلوب
...
if($chat_id == $sudo or $chat_id == 5975553574 ) {
  bot('EditMessageText',[
    ...
    'text'=>"
    *
   ارسل الان العدد ( ادني حد لتحويل النقاط(
    *
...
$RBEROO = [
  'inline_keyboard'=>[
  [['text'=>"قسم خدمات الرشق 💎",'callback_data'=>"service" ]],
  [['text'=>"تجميع نقاط 👤",'callback_data'=>"plus" ], ['text'=>"الحساب 🗃️",'callback_data'=>"acc" ]],
  [['text'=>"ادخال كود هدية 🎁",'callback_data'=>"hdia" ], ['text'=>"تحويل نقاط ♻️",'callback_data'=>"transer" ]],
  [['text'=>"معلومات طلب 📤",'callback_data'=>"infotlb" ], ['text'=>" الشـروط و الخصوصيه 📮",'callback_data'=>"shroth" ]],
  [['text'=>"قناه البوت 📛",'url'=>"$chabot.t.me" ], ['text'=>"الدعم الفني 📞",'url'=>"https://t.me/t4lga" ]],
  [['text'=>"شرح البوت 📣",'callback_data'=>"iykgvgg"]],
...
- الامان والثقه الموضوع الاول لدينا وحماية خصوصية جميع المستخدمين من الاولويات لدينا لذالك جميع المعلومات من النقاطوالطلبات هي محصنة تماماً لا يسمح لـ اي شخص الاطلاع عليها الا في حالة طلب المستخدم ذالك من الدعم الفني
...
else 
{
   ...
   'chat_id'=>$chat_id,
   'text'=>"
يمكنك تحويل نقاطاكثر من $AKTHAR فقط
  ", 
  'parse_mode'=>"markdown",
  'reply_markup'=>json_encode([
     'inline_keyboard'=>[
...
🗃️ الحساب
نقاطك : $coin نقطه
النقاطالمستخدمة : (".($rshq["cointlb"][$from_id] ?? "0").") نقطه
لقد دعوت : $share ✳️
...
if($data == "mstqbll") {
	...
	$wsfer = "يمكنك الرشق ✅" ;
		...
		$ster = "مقفل ❌" ;
		$wsfer = "لايمكنك الرشق حاليا اجمع نقاطلحد ما ينفتح ❌" ;
		} 
bot('EditMessageText',[
'chat_id'=>$chat_id,
'message_id'=>$message_id,
...
if(preg_match('/start Bero/',$text)){
   ...
   'chat_id'=>$chat_id,
   'text'=>"
  لقد حصلت علي *". $rshq['thoiler'][$e1]["coin"]. "* نقاطمن رابط التحويل
  ", 
  'parse_mode'=>"markdown",
  'reply_markup'=>json_encode([
     'inline_keyboard'=>[
...
bot('sendMessage',[
 ...
 وتم تحويل". $rshq['thoiler'][$e1]["coin"]." نقاطلحسابه
  ", 
  'parse_mode'=>"markdown",
  'reply_markup'=>json_encode([
     'inline_keyboard'=>[
...
نقاطك : ". $rshq["coin"][$from_id]. "
- - - - - - - - - - - - - - - - - - 
  ", 
 'parse_mode'=>"markdown",
 'reply_markup'=>json_encode([
     'inline_keyboard'=>[
     [['text'=>"ترجيع نقاطك",'callback_data'=>"ins|$from_id|". $rshq['coinn']]],
     [['text'=>"طلب تعويض تلقائيا",'callback_data'=>"tEwth|$idreq"]],
     [['text'=>"تصفير نقاطك",'callback_data'=>"msft|$from_id"]],
...
- - - - - - - - - - - - - - - - - - 
 ...
 'reply_markup'=>json_encode([
     'inline_keyboard'=>[
     [['text'=>"ترجيع نقاطك",'callback_data'=>"ins|$from_id|". $e[3]]],
...
تم ارجاع $e[2] نقاطلحساب [$e[1]](tg://user?id=$e[1])
